...
func newCategory(db *gorm.DB, opts ...gen.DOOption) category {
	...
	_category.DisplayName = field.NewString(tableName, "display_name")
	_category.Description = field.NewString(tableName, "description")
	_category.IconURL = field.NewString(tableName, "icon_url")
	_category.ColorCode = field.NewString(tableName, "color_code")
...
type category struct {
	...
	ALL         field.Asterisk
	ID          field.Int32  // カテゴリID
	Name        field.String // カテゴリの識別子（英語、小文字、アンダースコア区切り）
	DisplayName field.String // カテゴリの表示名（ユーザーに表示される名前）
	Description field.String // カテゴリの説明
	IconURL     field.String // カテゴリを表すアイコンのURL
	ColorCode   field.String // カテゴリの色コード（UI表示用）
...
func (c *category) updateTableName(table string) *category {
	...
	c.ColorCode = field.NewString(table, "color_code")
...
func (c *category) fillFieldMap() {
	...
	c.fieldMap["color_code"] = c.ColorCode
